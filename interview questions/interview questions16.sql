...
with cte as(
   select *,
          ...
          lag(sales_amount,1,0)over(partition by store_name order by month(sale_date))as last_month_sale 
from sales)
  select store_name,sales_amount,running_total,
        round(((sales_amount-last_month_sale)/last_month_sale)*100,2) as growth_ratio
from cte;
