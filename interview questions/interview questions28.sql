...
-- Question
/*
SQL Question : Average Duration of Employee's Service
...
/*
SQL Question : Average Duration of Employee's Service
can you find the average duration of service for employees across different departments? 
The Duration of service is represented as end_date - start_date. 
If end_date is NULL, consider it as the current date.
*/
...
with cte as
           (select *,
                   case when end_date is not null then datediff(end_date,start_date)
			            when end_date is null then datediff(current_date(),start_date) 
                   end as duration
		    from employee_service
            )
select department,round(avg(duration),2)as avg_duration 
from cte
group by department;
